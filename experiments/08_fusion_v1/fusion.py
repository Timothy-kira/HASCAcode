# %% [markdown]
# # WEAR 2026 — Step C: IMU + VideoMAE fusion network (CPU)
...
for k, (tri, vai) in enumerate(GroupKFold(5).split(tr_meta, y, sbj)):
    t0 = time.time(); torch.manual_seed(k)
    ...
    oof[vai] = predict(model, Xe[vai], eval_loc[vai], tr_vid[vai])
    pte += predict(model, Xte, te_loc, te_vid) / 5
...
np.save(f"{OUT}/oof_nn.npy", oof); np.save(f"{OUT}/te_nn.npy", pte)
