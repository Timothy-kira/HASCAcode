# %% [markdown]
# # WEAR 2026 — Step B v2: LightGBM base #2 (lr 0.03, stronger regularisation, valid-sensor sampling)
# Keeps the v1 evaluation sensor per tile (eval_loc from the baseline kernel) so all bases/pairs stay aligned.
...
eval_loc = np.load(glob.glob("/kaggle/input/**/eval_loc.npy", recursive=True)[0])  # same view as base v1
...
# %%
params = dict(objective="multiclass", num_class=19, learning_rate=0.03, num_leaves=31, min_data_in_leaf=100,
              feature_fraction=0.3, bagging_fraction=0.8, bagging_freq=1, lambda_l2=1.0, verbose=-1, num_threads=os.cpu_count())
...
for k, (tri, vai) in enumerate(GroupKFold(5).split(Xe, y, sbj)):
    ...
    m = lgb.train(params, dtr, 1500, valid_sets=[dva], callbacks=[lgb.early_stopping(150, verbose=False)])
...
# %%
np.save(f"{OUT}/oof_lgb2.npy", oof); np.save(f"{OUT}/te_lgb2.npy", pte)
